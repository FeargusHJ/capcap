...
while (($filename = readdir($dh)) !== false) 
{
	if (substr($filename, 0, 8) == "redcap_v" && is_dir(dirname(__FILE__) . DIRECTORY_SEPARATOR . $filename)) 
	{
		...
		// Make sure the cron.php file exists in the version folder
		if (!file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . "redcap_v" . $this_version . DIRECTORY_SEPARATOR . "cron.php")) continue;
...
include dirname(__FILE__) . DIRECTORY_SEPARATOR . "redcap_v" . $this_version . DIRECTORY_SEPARATOR . "cron.php";
